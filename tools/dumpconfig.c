				 ...
				 const char *dest_path)
{
	...
	/* Sync temp file to disk before rename to prevent data loss on crash */
	{
...
int editconfig_cmd(struct cmd_context *cmd, int argc, char **argv)
{
	...
	tree_spec.cmd = cmd;
	dm_list_init(&removals);

	/* First pass: build config string for additive edits */
	if (!dm_pool_begin_object(cmd->mem, 256))
		return_ECMD_FAILED;
	...
	dm_list_iterate_items(group, &cmd->arg_value_groups) {
		...
		ret = _edit_to_config_string(cmd->mem, edit_str);
		if (ret < 0) {
			dm_pool_abandon_object(cmd->mem);
			return EINVALID_CMD_LINE;
		}
	...
	dm_pool_grow_object(cmd->mem, "\0", 1);
	config_string = dm_pool_end_object(cmd->mem);

	/* Second pass: collect removal specs */
	dm_list_iterate_items(group, &cmd->arg_value_groups) {
		if (!grouped_arg_is_set(group->arg_values, edit_ARG))
			continue;
		...
		if (strchr(edit_str, '=') && !strcmp(strchr(edit_str, '=') + 1, "-")) {
			if (!_parse_removal(cmd->mem, edit_str, &removals))
				return EINVALID_CMD_LINE;
		}
	}
	...
	/* Apply additive edits via cascade + flatten */
	if (has_edits) {
		if (!(edits_cft = dm_config_from_string(config_string))) {
	...
	/* Apply removals */
	if (!dm_list_empty(&removals))
		_apply_removals(cft, &removals);
